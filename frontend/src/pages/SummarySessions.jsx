...
import { BarChart3, ChevronRight, CalendarDays, Layers, ShieldCheck } from "lucide-react";
import { useAuth } from "../context/AuthContext";
import { attendanceApi } from "../api/endpoints";

export default function SummarySessions() {
        ...
        <p className="text-xs text-slate-500 dark:text-slate-300 mt-0.5 flex items-center gap-1.5">
          {isAdmin ? (
            <>
              ...
              Admin view — analytics across every session
            </>
          ) : (
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {sessions.map((s) => (
            <button
              key={s.name}
              onClick={() => navigate(`/attendance/summary/${encodeURIComponent(s.name)}`)}
